...
ใน DB คอลัมน์ `image_path` จะเก็บ **public URL** (ถ้ามี `R2_PUBLIC_URL`) หรือ **object key** (`meals/{mealId}.jpg`)  
รูปเก่าที่เป็นแค่ `uuid.jpg` ยังอ่านได้ (map เป็น `meals/uuid.jpg`)
